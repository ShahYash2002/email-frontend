...
export default {
  setup() {
    ...
    return { v$: useVuelidate(), type: route.params.type };
  },
  data() {
    return {
      username: "Yash Shah",
  ...
  validations() {
    return {
      username: {
        required: helpers.withMessage("Userame cannot be empty", required),
        alphaNum: helpers.withMessage(
          "Userame must be alpha numeric",
          alphaNum
        ),
      },
      type: {
  ...
  methods: {
    handleSubmit: async function () {
      ...
      clientApi
        .addClient(this.username, this.type)
        .then((result) => {
          console.log(result);
